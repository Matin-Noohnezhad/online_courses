...
def train(controller_net, soft_1, soft_2, layers_types, skip_connections, policy_gradient_multiplier, grad_clip_value=0.25,
    ...
    # optimizer = optim.Adam(controller_net.parameters(), lr=9e-2)
    ...
    for epoch in range(no_epoch):
        ...
        soft_2 = soft_2.reshape(soft_2.shape[0], soft_2.shape[2])
        ####################################### dirty codes #######################################
        loss1 = criterion(soft_1, layers_types)
        loss2 = criterion(soft_2, skip_connections)
        ...
        total_loss.backward(torch.ones_like(total_loss) * (policy_gradient_multiplier/100))
        # total_loss.backward()
